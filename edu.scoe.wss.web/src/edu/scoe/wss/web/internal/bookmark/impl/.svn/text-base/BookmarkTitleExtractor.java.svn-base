...
public class BookmarkTitleExtractor {
    ...
    private static final Pattern TITLE_TAG =
        Pattern.compile("\\<title>(.*)\\</title>", Pattern.CASE_INSENSITIVE|Pattern.DOTALL); //$NON-NLS-1$
    ...
    public static String getPageTitle(String url) throws IOException {
        ...
        if (!contentType.contentType.equals("text/html")) //$NON-NLS-1$
            ...
            if (matcher.find()) {
                ...
                return matcher.group(1).replaceAll("[\\s\\<>]+", " ").trim(); //$NON-NLS-1$ //$NON-NLS-2$
            }
            else
                return null;
        }
    ...
    private static ContentType getContentTypeHeader(URLConnection conn) {
        ...
        do {
            ...
            if (headerName != null && headerName.equals("Content-Type")) //$NON-NLS-1$
    ...
    private static final class ContentType {
        private static final Pattern CHARSET_HEADER = Pattern.compile("charset=([-_a-zA-Z0-9]+)", Pattern.CASE_INSENSITIVE|Pattern.DOTALL); //$NON-NLS-1$
 
        private String contentType;
        private String charsetName;
        private ContentType(String headerValue) {
            if (headerValue == null)
                throw new IllegalArgumentException("ContentType must be constructed with a not-null headerValue"); //$NON-NLS-1$
            int n = headerValue.indexOf(";"); //$NON-NLS-1$
            if (n != -1) {
